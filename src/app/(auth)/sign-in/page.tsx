...
import { HeartPulse, Eye, EyeOff, Sun, Moon, User, Stethoscope, Building2, Store, Shield } from 'lucide-react';
...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await signIn(email, password, selectedRole, selectedRole === 'admin' ? authCode : undefined);
    ...
    <div className="min-h-screen flex" style={{ background: isLight ? '#E5E5E9' : '#151519' }}>
      ...
      <div className="flex-1 flex items-center justify-center px-6 relative z-10">
        <div className="w-full max-w-sm">
          <h1 className="text-2xl font-bold mb-1 heading-text" style={{ color: isLight ? '#1D1D1F' : '#F8FAFC' }}>Welcome back</h1>
          <p className="text-sm mb-6" style={{ color: isLight ? '#86868B' : '#9CA3AF' }}>Select your role to continue</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-sm font-medium mb-1.5 block" style={{ color: isLight ? '#3A3A3C' : '#D1D5DB' }}>{selectedRole === 'admin' ? 'Admin Username' : 'Email'}</label>
              <input
                type={selectedRole === 'admin' ? 'text' : 'email'}
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder={selectedRole === 'admin' ? 'Enter admin username' : 'you@example.com'}
                className="w-full px-3.5 py-2.5 rounded-lg text-sm border outline-none transition-all glass-card"
                style={{ color: isLight ? '#1D1D1F' : '#F8FAFC' }}
                required
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-3.5 py-2.5 pr-10 rounded-lg text-sm border outline-none transition-all glass-card"
                  style={{ color: isLight ? '#1D1D1F' : '#F8FAFC' }}
                  required
                />
                <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color: isLight ? '#86868B' : '#6B7280' }}>
            ...
            {selectedRole === 'admin' && (
              <div>
                <label className="text-sm font-medium mb-1.5 block" style={{ color: isLight ? '#3A3A3C' : '#D1D5DB' }}>Auth Code</label>
                <input
                  type="password"
                  value={authCode}
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center gap-2" style={{ color: isLight ? '#86868B' : '#6B7280' }}>
                <input type="checkbox" className="rounded" style={{ accentColor: '#2563EB' }} /> Remember me
              </label>
              <Link href="/forgot-password" className="font-medium hover:opacity-80" style={{ color: '#2563EB' }}>Forgot password?</Link>
            </div>
      ...
      <div className="hidden lg:flex flex-1 items-center justify-center p-12 relative z-10" style={{ background: isLight ? 'rgba(220,220,226,0.6)' : 'rgba(15,15,20,0.6)', backdropFilter: 'blur(12px)' }}>
